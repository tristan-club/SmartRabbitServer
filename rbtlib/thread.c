...
static void * thread_routine (void *arg)
{
	...
	while (1)
	{
		...
		pool->cur_queue_size--;
		ThreadJob *worker = pool->queue_head;
...
int rbtT_add_job ( ThreadPool * pool , void *(*process) (void *arg), void *arg)
{
	/*构造一个新任务*/
	ThreadJob *newworker =
		(ThreadJob *) malloc (sizeof (ThreadJob));
	...
	pthread_mutex_lock (&(pool->queue_lock));
	/*将任务加入到等待队列中*/
	ThreadJob *member = pool->queue_head;
	if (member != NULL)
	{
...
{
	ThreadPool * pool = rbtT_init (3);/*线程池中最多三个活动线程*/
	...
	{
		workingnum[i] = i;
		rbtT_add_job( pool , myprocess, &workingnum[i]);
	}
